      ...
      <v-col cols="12" lg="8">
        <div class="resource-hero__title-row">
          <div class="resource-hero__icon">
            <v-icon icon="mdi-server-outline" size="32" />
          </div>
          <div>
            <h1 class="resource-hero__title">{{ $t('pages.services') }}</h1>
            <p class="resource-hero__subtitle">
              聚合服务监听、TLS 关联与协议依赖，入口更集中，扫一眼就能知道状态。
            </p>
          </div>
        </div>
...
const inTags = computed((): any[] => {
  return [...Data().inbounds?.map((o:any) => o.tag).filter(t => t != null), ...Data().endpoints?.filter((e:any) => e.listen_port > 0 && e.type != "masque" && e.type != "mieru").map((e:any) => e.tag)]
})
