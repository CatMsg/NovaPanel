export default {
  ...
  main: {
    ...
    infos: "信息",
    hero: {
      badge: "NovaPanel 控制中心",
      title: "一眼掌握全局",
      subtitle: "系统状态、Sing-Box 运行情况和常用操作，都放在这里。",
      live: "实时状态",
    },
    gauge: {
      cpu: "CPU 仪表",
  ...
  setting: {
    ...
    loginBanAllowlist: "登录封禁白名单",
    loginBanAllowlistHint: "不会被永久封禁的管理来源 IP / CIDR，使用逗号、空格或换行分隔。",
    ...
    subModeMaster: "主模式",
    subModeSlave: "从模式",
    subMasterSources: "主模式上游订阅地址",
    subMasterSourcesHint: "每行填写一个上游订阅链接，系统只会合并这些上游节点。",
    subAggregateUri: "聚合订阅 URI",
    jsonSub: "JSON 订阅",
    toDirect: "路由到直连",
    toBlock: "路由到阻止",
    timestamp: "时间戳",
  ...
  client: {
    ...
    filters: "筛选条件",
    bulkActions: "批量操作",
    manageDesc: "管理用户、批量调整、查看历史与在线状态，桌面端与移动端保持一致的操作入口。",
    manageHint: "集中查看用户状态、流量、到期时间和常用操作。",
    total: "总数",
    showing: "当前显示",
    offline: "离线",
    exhausted: "流量耗尽",
    ...
    uploadLimit: "上传限速",
    downloadLimit: "下载限速",
    rateLimitHint: "0 表示不限速；同一用户的所有入站和设备共享此带宽",
  ...
  types: {
    ...
    wg: {
      ...
      peer: "对等体",
      peers: "对等体",
      guideTitle: "创建 WireGuard 的顺序",
      guide: "先确认本机密钥、隧道地址和 UDP 监听端口，再添加客户端。保存后可在节点卡片中扫码或下载客户端配置。",
      localSection: "本机节点",
      localSectionHint: "WireGuard 服务端的身份和监听设置",
      privateHint: "点击右侧钥匙按钮可生成新的密钥对",
      publicHint: "由私钥计算，用于客户端识别本机节点",
      localIpHint: "本机在隧道内的地址，例如 10.10.0.1/24",
      listenPortHint: "需要在系统防火墙和云安全组中放行此 UDP 端口",
      dnsHint: "写入导出的客户端配置，例如 1.1.1.1, 9.9.9.9",
      systemHint: "通常保持关闭，仅在需要操作系统接口时开启",
      clientsSection: "客户端",
      clientsSectionHint: "每个客户端使用独立密钥和隧道 IP",
      addClient: "添加客户端",
      noClients: "尚未添加客户端。添加后才能生成二维码和客户端配置。",
      clientPrivateKey: "客户端私钥",
      clientPrivateHint: "仅用于导出客户端配置，请妥善保管",
      clientPublicKey: "客户端公钥",
      clientPublicHint: "本机通过该公钥识别客户端",
      optionalHint: "可选，不使用时留空",
      peerAddress: "客户端公网地址",
      peerAddressHint: "客户端没有固定公网地址时留空",
      peerPort: "客户端公网端口",
      peerPortHint: "客户端没有固定公网端口时留空",
      keepAliveHint: "客户端位于 NAT 后时建议设置为 25 秒",
      clientTunnelIp: "客户端隧道 IP",
      allowedIpHint: "分配给该客户端，例如 10.10.0.2/32",
    },
    lb: {
      defaultOut: "默认出站",
