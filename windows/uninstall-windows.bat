...
echo ========================================
echo NovaPanel Windows Uninstaller
echo ========================================
...
echo Uninstalling NovaPanel from: %INSTALL_DIR%
...
if exist "%INSTALL_DIR%\novas-service.exe" (
    ...
    cd /d "%INSTALL_DIR%"
    novas-service.exe uninstall >nul 2>&1
    if %errorLevel% equ 0 (
...
if exist "%DESKTOP%\NovaPanel.lnk" (
    del "%DESKTOP%\NovaPanel.lnk" >nul 2>&1
    echo Desktop shortcut removed
)

REM Remove Start Menu shortcut
echo Removing Start Menu shortcut...
set "START_MENU=%APPDATA%\Microsoft\Windows\Start Menu\Programs\NovaPanel"
if exist "%START_MENU%" (
...
reg delete "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v NOVAS_HOME /f >nul 2>&1
...
if /i "%keep_data%"=="y" (
    ...
    if exist "%INSTALL_DIR%\novas.exe" del "%INSTALL_DIR%\novas.exe" >nul 2>&1
    if exist "%INSTALL_DIR%\novas-service.exe" del "%INSTALL_DIR%\novas-service.exe" >nul 2>&1
    if exist "%INSTALL_DIR%\novas-service.xml" del "%INSTALL_DIR%\novas-service.xml" >nul 2>&1
...
netsh advfirewall firewall delete rule name="NovaPanel Panel" >nul 2>&1
netsh advfirewall firewall delete rule name="NovaPanel Subscription" >nul 2>&1
...
echo ========================================
echo.
echo NovaPanel has been uninstalled from your system.
echo.
if /i "%keep_data%"=="y" (
...
echo.
echo Thank you for using NovaPanel! (compatibility files are cleaned, core data may be preserved if you chose to keep it)
echo.
pause
