...
func (s *ClashService) ConvertToClashMeta(outbounds *[]map[string]interface{}, basicConfig string) (string, error) {
	...
	for _, obMap := range *outbounds {
		...
		case "masque":
			proxy["type"] = "masque"
			copyClashFields(proxy, obMap, "network", "private-key", "public-key", "ip", "mtu", "udp", "remote-dns-resolve", "dns", "congestion-controller", "proto", "keepalive")
		...
		case "mieru":
			proxy["type"] = "mieru"
			copyClashFields(proxy, obMap, "port-range", "transport", "udp", "username", "password", "multiplexing", "handshake-mode", "traffic-pattern", "mtu")
		case "tailscale":
